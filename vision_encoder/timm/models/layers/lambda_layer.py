...
class LambdaLayer(nn.Module):
    ...
    def __init__(
        self, dim, dim_out=None, stride=1, num_heads=4, dim_head=16, r=7, qkv_bias=False
    ):
        super().__init__()
        self.dim = dim
        ...
        self.conv_lambda = nn.Conv3d(
            1, dim_head, (r, r, 1), padding=(r // 2, r // 2, 0)
        )
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x)
        q, k, v = torch.split(
            qkv, [self.num_heads * self.dim_k, self.dim_k, self.dim_v], dim=1
        )
        q = (
            self.norm_q(q).reshape(B, self.num_heads, self.dim_k, M).transpose(-1, -2)
        )  # B, num_heads, M, K
        ...
        out = (
            (content_out + position_out).transpose(3, 1).reshape(B, C, H, W)
        )  # B, C (num_heads * V), H, W
        out = self.pool(out)
        return out
