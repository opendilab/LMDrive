...
class PosEmbedRel(nn.Module):
    ...
    def __init__(self, feat_size, dim_head, scale):
        ...
        self.height_rel = nn.Parameter(
            torch.randn(self.height * 2 - 1, dim_head) * self.scale
        )
        self.width_rel = nn.Parameter(
            torch.randn(self.width * 2 - 1, dim_head) * self.scale
        )
...
class BottleneckAttn(nn.Module):
    ...
    def __init__(
        self, dim, dim_out=None, feat_size=None, stride=1, num_heads=4, qkv_bias=False
    ):
        super().__init__()
        assert (
        ...
        self.pos_embed = PosEmbedRel(
            feat_size, dim_head=self.dim_head, scale=self.scale
        )
    ...
    def forward(self, x):
        ...
        attn_out = (
            (attn_out @ v).transpose(1, 2).reshape(B, self.dim_out, H, W)
        )  # B, dim_out, H, W
