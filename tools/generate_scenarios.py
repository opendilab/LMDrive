...
def main(args):
    ...
    for curr_transform in available_event_configurations_dict_list:
        ...
        augmented_transforms += add_yaw_augmentation(
            curr_transform
        )  # add base transform
    ...
    for town in towns:
        ...
        for index in range(1, 11):
            if (
                index == 2 or index == 5 or index == 6
            ):  # discard lane changing scenarios since expert is not configured for it
                continue
            all_scenarios.append(
                {
                    "available_event_configurations": aggr_available_event_config,
                    "scenario_type": "Scenario%d" % index,
                }
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="original file reference scenarios",
    )
    parser.add_argument(
        "--save_file", type=str, required=False, help="output file with dense scenarios"
    )
    parser.add_argument(
        "--towns",
        type=str,
        required=True,
        help="Town01/Town02/Town03/Town04/Town05/Town06/all",
    )
